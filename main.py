...
TTL = (
    60 * 60 * 24 * config["listing"]["ttl"]
)  # expiry (in days) of listing redis entry (interpreted as "message already sent")
...
def generate_message(listing: CarousellListing) -> str:
    ...
    title = (
        t
        if len((t := listing.title)) < TITLE_LIMIT
        else f"{t[:TITLE_LIMIT].strip()}..."
    )
    description = (
        d if len((d := listing.description)) else f"{d[:DESCRIPTION_LIMIT].strip()}..."
    )
...
def run_script(dry_run=True):
    ...
    for brand in random.sample(BRANDS, 3):
        search = CarousellSearch(brand).filter(
            collections=CarousellCategories.ALL_MENS_FASHION
        )
        ...
        print(
            f"Got {unfiltered_len} listings under {brand}; {len(listings)} are recent"
        )
